...
export default class {
  constructor(log, config, api) {
    ...
    try {
      ...
      // Either an accessToken (NEO 2.0 JWT auth) or both username and password are required
      if (!config.countryCode || (!config.accessToken && (!config.username || !config.password))) {
  ...
  async pluginSetup() {
    ...
    try {
      ...
      // Determine the seed for the EcoVacsAPI device ID:
      // Use the accessToken for NEO 2.0, otherwise the configured username
      const deviceIdSeed = this.config.accessToken
        ? this.config.accessToken
        : this.config.username

      // Connect to Ecovacs/Yeedi
      this.ecovacsAPI = new EcoVacsAPI(
        EcoVacsAPI.getDeviceId(this.api.hap.uuid.generate(deviceIdSeed)),
      ...
      // Determine device list based on authentication method
      let deviceList
      if (this.config.accessToken) {
        // NEO 2.0 JWT authentication - parse the token and skip traditional login
        const jwtPayload = this.parseJwtPayload(this.config.accessToken)
        this.neo2Config = {
          clientId: jwtPayload.c,
          continent: countries[this.config.countryCode].continent.toLowerCase(),
          resource: jwtPayload.r,
          userId: jwtPayload.u,
        }

        // Set EcoVacsAPI auth state directly from the JWT
        this.ecovacsAPI.uid = jwtPayload.u
        this.ecovacsAPI.resource = jwtPayload.r
        this.ecovacsAPI.user_access_token = this.config.accessToken

        this.log('%s.', platformLang.neo2JwtAuth)

        // Schedule automatic token refresh before the access token expires (if expiry is present)
        if (jwtPayload.exp && this.config.refreshToken) {
          this.scheduleNeo2TokenRefresh(jwtPayload.exp)
        }

        // Fetch the device list using JWT bearer token authentication
        deviceList = await this.getNeo2Devices()
      } else {
        // Traditional Ecovacs/Yeedi username and password authentication
        try {
        ...
        deviceList = await this.ecovacsAPI.devices()
      }
  ...
  pluginShutdown() {
    ...
    try {
      ...
      // Clear the NEO 2.0 token refresh timer if active
      if (this.neo2RefreshTimer) {
        clearTimeout(this.neo2RefreshTimer)
      }
  ...
  parseJwtPayload(token) {
    ...
    if (parts.length !== 3) {
      throw new Error(platformLang.invalidJwt)
    }
  ...
  scheduleNeo2TokenRefresh(expiryUnixSeconds) {
    // Clear any existing timer
    if (this.neo2RefreshTimer) {
      clearTimeout(this.neo2RefreshTimer)
    }
    // Refresh 1 hour before expiry; wait at least 60 seconds before the first attempt
    const refreshBeforeExpirySeconds = 3600
    const minRefreshDelayMs = 60000
    const nowSeconds = Math.floor(Date.now() / 1000)
    const refreshInMs = Math.max(
      (expiryUnixSeconds - nowSeconds - refreshBeforeExpirySeconds) * 1000,
      minRefreshDelayMs,
    )
    this.neo2RefreshTimer = setTimeout(() => this.refreshNeo2Token(), refreshInMs)
    this.log.debug('[NEO2] Token refresh scheduled in %d seconds.', Math.round(refreshInMs / 1000))
  }

  async refreshNeo2Token() {
    // Refresh the NEO 2.0 JWT access token using the stored refresh token
    try {
      const { clientId, continent, userId } = this.neo2Config
      const apiBase = `https://api-app.dc-${continent}.ww.ecouser.net/api`
      ...
      const res = await fetch(`${apiBase}/appsvr/oauth/token?${params}`, {
        headers: {
          appid: 'ecovacs',
          token: '',
          userid: userId,
      ...
      // Update stored tokens and the EcovacsAPI instance
      this.config.accessToken = data.data.access_token
      this.config.refreshToken = data.data.refresh_token
      this.ecovacsAPI.user_access_token = data.data.access_token

      // Schedule the next refresh based on the new token's expiry
      const newPayload = this.parseJwtPayload(data.data.access_token)
      if (newPayload.exp) {
        this.scheduleNeo2TokenRefresh(newPayload.exp)
      }

      this.log('%s.', platformLang.neo2TokenRefreshed)
    } catch (err) {
      this.log.warn('%s %s.', platformLang.neo2TokenRefreshFail, parseError(err))
    }
  }

  async getNeo2Devices() {
    // Fetch the device list from the Ecovacs API using JWT bearer token authentication
    const { continent, userId } = this.neo2Config
    const apiBase = `https://api-app.dc-${continent}.ww.ecouser.net/api`

    const res = await fetch(`${apiBase}/appsvr/app.do`, {
      body: JSON.stringify({
    ...
    if (data.code === -1000) {
      await this.refreshNeo2Token()
      return this.getNeo2Devices()
    }

    // Check if any of the known success indicators are present (mirrors the ecovacs-deebot library convention)
    if ((data.result !== 'ok') && (data.ret !== 'ok') && (data.msg !== 'success')) {
      throw new Error(`${platformLang.deviceListFail}: ${data.error || data.msg || JSON.stringify(data)}`)
    }
